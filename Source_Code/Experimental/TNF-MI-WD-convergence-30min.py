...
pps_list = np.linspace(10,500,50)
...
for key in all_results:
    ...
    axs[0].plot(pps_list, W2_vals, 'o-', label=label)
    axs[0].axhline(epsilon, color='k', linestyle='--', lw=1)
# axs[0, 1].set_yscale("log")
axs[0].set_title("2-WD vs points_per_std (30min)")
axs[0].grid(True)
axs[0].legend()
...
for key in all_results:
    label = key.replace('X-meanYcX-stdYcX-', '').replace('.txt', '')
    axs[1].plot(pps_list, all_results[key]['n_y'], 'o-', label=label)
axs[1].set_title("n_y vs points_per_std (30min)")
axs[1].set_xlabel("points_per_std")
axs[1].grid(True)
axs[1].legend()
...
for key in all_results:
    ...
    axs[0].plot(pps_list, MI_vals, 'o-', label=label)
axs[0].set_title("MI vs points_per_std (30min)")
axs[0].grid(True)
axs[0].legend()
...
for fname, result in all_results.items():
    ...
    convergence_array = np.column_stack((pps_list, result['n_y'], result['MI']))
    np.savetxt(
        f'{outdir}/{label_clean}_pps-ny-MI.dat',
        convergence_array,
        header='points_per_std\tn_y\tMI',
        fmt='%.6e',
        delimiter='\t'
    )
